...
import { uploadVideo, uploadBackgroundImage, startVideoSegmentation, createTask, getAllBackgrounds, getAllVideos, uploadBackgroundToLibrary } from '../../services/videoService';
...
const VideoUpload = () => {
    ...
    if (videoRef.current && videoPreview) {
      videoRef.current.onloadedmetadata =  () => {
        ...
        if(selectedVideo && !videoId) {
          //如果是新上传的视频 需要将视频上传视频库
          uploadVideo(selectedVideo).then((uploadedVideo) => {
            const currentVideoId = uploadedVideo.video.id;
  ...
  const handleBackgroundChange = async (event) => {
    const file = event.target.files[0];
    setBackgroundId(null); // 清除之前的背景ID
    if (file) {
      try {
        setUploadingBackground(true);
        const uploadedBackground = await uploadBackgroundToLibrary(file);
  ...
  const handleStartTask = async () => {
    ...
    try {
      ...
      if (selectedBackground && applyBackground && backgroundId) {
        await uploadBackgroundImage(currentVideoId, backgroundId);
      }
      // console.log('上传背景图片成功', selectedBackground, backgroundId);
      return
